...
async function init() {
    ...
    if (localScores.length === 0) {
        localScoreElement.innerText = `ليس لديك اي نتيجة على  "${niceMap}" بعد `;
    } else {
    ...
    if (globalHighScores.length === 0) {
        globalScoreElement.innerText = `ليس هناك نتائج على  "${niceMap}" بعد`;
    } else {
        ...
        html += `
            <li class="score" onclick="showScore(event)">
                ...
                <div class="hidden">
                    ...
                    <div class="rules">
                        <h4>قوانين اللعبة</h4>
                        <p>الجولات: ${score.rules.roundCount}</p>
                        <p>حد الوقت: ${score.rules.timeLimit === -1 ? "∞" : score.rules.timeLimit}<p>
                        <p>حد الحركة: ${score.rules.moveLimit === -1 ? "∞" : score.rules.moveLimit}<p>
                        <p hidden>Zoom: ${score.rules.zoomAllowed ? "allowed" : "restricted"}<p>
                        <p hidden>Panning: ${score.rules.panAllowed ? "allowed" : "restricted"}<p>
                    </div>
                </div>
            </li>
        `;
